...
const WaveCursor = () => {
  ...
  useEffect(() => {
    // Detect mobile/touch device - CRITICAL for performance
    const checkMobile = () => {
    ...
    const draw = () => {
      // EFECTO DE ESTELA RESTAURADO (Fondo semi-transparente)
      ctx.fillStyle = 'rgba(2, 6, 23, 0.2)'; // slate-950 con transparencia
      ...
      for (let i = 0; i < config.lineCount; i++) {
        ...
        for (let x = 0; x <= width; x += config.resolution) {
            ...
            // ONDA COMPLEJA (SIN + COS) RESTAURADA
